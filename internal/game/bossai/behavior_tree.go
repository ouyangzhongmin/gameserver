...
// CreateBasicCombatTree 创建基础战斗行为树
func CreateBasicCombatTree() *BehaviorTree {
	return NewBehaviorTreeBuilder("BasicCombat").
		Selector("Combat Selector").
		Sequence("Attack Sequence").
		Condition(NewHasTargetCondition()).
		Action(NewMoveToTargetNode(50.0)).
...
// CreateAdvancedCombatTree 创建高级战斗行为树
func CreateAdvancedCombatTree() *BehaviorTree {
	return NewBehaviorTreeBuilder("AdvancedCombat").
		Selector("Main Selector").
		// 低血量时的特殊行为
		Sequence("Low Health Behavior").
		...
		// 攻击序列（带冷却）
		Cooldown("Attack Cooldown", time.Second*2).
		Sequence("Attack Sequence").
		Action(NewMoveToTargetNode(50.0)).
		Action(NewAttackTargetNode()).
		End().
		...
		End().
		// 寻找目标
		Action(NewFindTargetNode(300.0)).
		End().
		Build()
}
